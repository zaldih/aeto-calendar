...
export class MainController {
  constructor(private aetoService: AetoService,
  ...
  private async downloadEvents(urls: string[]): Promise<string[]> {
    ...
    return await Promise
        .all(urls.map(
            async (
                url) => { return await this.aetoService.downloadFile(url); }))
        .then((data) => {
          console.log("Activities PDF: ", data.map((path) => basename(path)));
          console.log("Done");
          return data;
        });
  }

  private async getEventsFromFiles(paths: string[]):
      Promise<(void|AETOEvent)[]> {
    console.log("Parsing activities...");
    return Promise
        .all(paths.map(async (path) => {
          return await this.eventService.getEventFromDocument(path).catch(
              (error) => { console.error("#getEventsFromFiles", error); });
        }))
        .then((data) => {
  ...
  private async addEventsToCalendar(events: any[]) {
    ...
    for await (const event of events) {
      await this.addEventToCalendar(event).catch(
          (error) => console.error("#addEventsToCalendar", error));
    }
  ...
  async test() {
    ...
    for (let i = 0; i < files.length; i++) {
      const event =
          await this.eventService.getEventFromDocument("data/" + files[i]);
      console.log(event);
    ...
    const similarExistingEvents =
        (await this.calendarService.get({
          ...
          timeMax : ADD_HOURS(event.endDate, 1),
            ...
            .data.items.filter((calendarEvent: any) =>
  ...
  private async removeDuplicateEvent(
      duplicatedEvents: CalendarEventFromGoogle[]): Promise<void> {
